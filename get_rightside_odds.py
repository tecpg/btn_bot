            ...
            for card in soup.find_all("div", class_="card"):
                try:
                   ...
                   # Extracting the text from the card
                    rightt_text = card.select_one(".row .col-6:nth-of-type(1)").get_text(strip=True).replace('\n', ' ')
                   
                    
                    # Regular expression to extract the odds (e.g., 3.42 from '2events@3.42 odds')
                    odds_match = re.search(r'@([\d.]+)', rightt_text)
                    ...
                    float_right = card.select_one("span.float-right")
                    from_code = float_right.contents[0].strip('@') if float_right and float_right.contents else ""
                    from_code =  from_code.replace('\r', '').replace('\n', '').strip()

                    from_platform = ""

                    if float_right:
                        code_elem = float_right.select_one("code")
                        if code_elem:
                    ...
                    flag_icon_elem = float_right.select_one("span.flag-icon") if float_right else None
                  ...
                  # List of allowed platforms
                    ...
                    try:
                        ...
                        price = 'premium' if numeric_odds > 100 else 'free'
                    ...
                    result = {
                        ...
                        "odd": odds,
                        "rate": rate,
                        "email": 'hello@bettingtipsnet.com',
                        "price": price,
                        "post_time": post_time,
                        "post_date": post_date,
