...
def scrape_hockey_data(set_date):
    all_rows = []
    ...
    with sync_playwright() as p:
        ...
        page.goto(f"https://www.forebet.com/en/hockey/predictions/{set_date}")
    ...
    for i, game in enumerate(games):
        ...
        try:
            ...
            if score_div:
                ...
                if len(spans) >= 2:
                    score1 = spans[0].get_text(strip=True)
                    score2 = spans[1].get_text(strip=True)
    ...
    try:
        ...
        with open(csv_f, "r") as f:
            ...
            for row in csv_data:
                if len(row) == 16:
                    cursor.execute(
                        'INSERT INTO hockey (league, fixtures, tip, odd, match_time, score, date, flag, result, code, source,rate, avg_point,correct_score,fixtures_link, league_slug)'
        ...
        cursor.execute('DELETE t1 FROM hockey AS t1 INNER JOIN hockey AS t2 WHERE t1.id < t2.id AND t1.fixtures = t2.fixtures AND t1.source = t2.source')
...
def main():
    print(p_date)
    rows = scrape_hockey_data(p_date)
    if rows:
        save_to_csv(rows)
        post_to_mysql()
    else:
